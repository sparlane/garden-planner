...
class CohortClosingStockTests(CohortStockTestCase):
    ...
    def test_the_report_closing_stock_moves_by_the_sold_cost(self):
        ...
        others = StockValuationLine.objects.filter(
            income_year=self.income_year,
        ).exclude(source_type='plant_cohort')
        self.assertEqual(after - sum((line.value for line in others), Decimal('0')), Decimal('0.8100'))
