...
def capture_cohort_lines(case):
    """Capture `case`'s income year and return its cohort lines by cohort.

    A plain function rather than a base class: every capture test wants this
    one call, and a shared `TestCase` between them and `CohortStockTestCase`
    would put the leaves one generation deeper than pylint allows.
    """
    capture_inventory(case.income_year, case.user)
    lines = StockValuationLine.objects.filter(
        income_year=case.income_year, source_type='plant_cohort',
    )
