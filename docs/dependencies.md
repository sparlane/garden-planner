...
| --- | --- |
| `common` utilities | Python and framework primitives only; no application modules. Shared query parsers live in `common.rest_query`, the catalog retirement rule in `common.retirement`, the catalog merge rule in `common.merging`, which builds on it, and the coded usage setting in `common.coded`, which builds on that. Nothing there may read a workspace, so what a catalog rule needs the current one to answer lives in `workspaces.scoping` instead: `CurrentWorkspaceCatalogSerializer` runs the model's own validation against the workspace a record would belong to, and names the record already holding a value that is unique inside it. |
